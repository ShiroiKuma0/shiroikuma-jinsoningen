...
// Upstream's own version literals (`versionName` and `versionCode` in defaultConfig below)
// stay untouched, so an upstream rebase brings the new base in by itself. Our fork tail
// derives from them, and AGP's generated BuildConfig.VERSION_NAME carries the result — the
// User-Agent and the Settings version row both read our version, not upstream's:
